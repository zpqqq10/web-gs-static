...
- open custom `.ply` / `.drc` files by adding a `url` param to a CORS-enabled URL, the `url` param can specify a path containing one `pc.ply` / `pc.drc` and one optional `camera.json`, or it can simple specify a path to a `.ply` / `.drc`
- drag and drop a `.ply` file which has been processed with the 3d gaussian splatting software onto the page
- change the `INPUT` variable in `data/strip.py` to discard bulky attributes
- support transferring and decoding `.drc` file, which is the compressed point cloud file format defined by [Draco](https://github.com/zpqqq10/gs-draco)

## examples

- https://zpqqq10.github.io/web-gs-static/
- https://zpqqq10.github.io/web-gs-static/?url=https://path/
- https://zpqqq10.github.io/web-gs-static/?url=https://path/truck.ply
...
## words of [Kevin Kwok](https://github.com/antimatter15/)
